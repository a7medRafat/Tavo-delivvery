...
class ChipsChoice<T> extends StatelessWidget {
  ...
  Widget _listScrollable(BuildContext context) {
    return SingleChildScrollView(
      ...
      child: Row(
        children: _choiceItems(context) as List<Widget>,
      ),
    );
  }
  ...
  _choiceItems(BuildContext context) {
    final ThemeData theme = Theme.of(context);
    return List<Widget>.generate(options.length, _choiceItemsGenerator(theme)).toList();
  }

  Widget Function(int) _choiceItemsGenerator(ThemeData theme) {
    return (int i) {
      ChipsChoiceOption<T> item = options[i];
      bool selected = _isMultiChoice ? _values!.contains(item.value) : _value == item.value;
      return itemBuilder?.call(item, selected, _select(item.value)) ??
          _ChipsChoiceItem(
            item: item,
            config: itemConfig,
            selected: selected,
            selectedColor: itemConfig.selectedColor ?? theme.cardColor,
...
class _ChipsChoiceItem<T> extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final bool isDark = selected ? config.selectedBrightness == Brightness.dark : config.unselectedBrightness == Brightness.dark;
    ...
    final double? borderOpacity = selected ? config.selectedBorderOpacity : config.unselectedBorderOpacity;

    final Color borderColor = isDark ? const Color(0x00000000) : textColor!.withOpacity(borderOpacity ?? .2);

    final Color? checkmarkColor = isDark ? textColor : selectedColor;

    final Color? backgroundColor = isDark ? unselectedColor : const Color(0x00000000);

    final Color? selectedBackgroundColor = isDark ? selectedColor : const Color(0x00000000);
    ...
    return Padding(
      ...
      child: FilterChip(
        ...
        avatar: item.avatar,
        shape: config.shapeBuilder?.call(selected) as OutlinedBorder? ??
            StadiumBorder(
        ...
        selected: selected,
        onSelected: item.disabled == false ? (_selected) => onSelect(_selected) : null,
      ),
    );
  }
}
...
typedef _ChipsChoiceOptionProp<T, R> = R Function(int index, T item);

/// Builder for chips shape border
typedef ChipsChoiceShapeBuilder = ShapeBorder Function(bool selected);

/// Callback when the value changed
typedef ChipsChoiceChanged<T> = void Function(T value);

/// Builder for custom choice item
typedef ChipsChoiceBuilder<T> = Widget Function(
